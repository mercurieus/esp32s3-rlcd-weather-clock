...
Based on the original [v-cu/esp32s3-rlcd-weather-clock](https://github.com/v-cu/esp32s3-rlcd-weather-clock).

**Calendar + clock** — month grid, analogue dial, four-day forecast and a quote strip:

![Simulated calendar screen](img/screen-calendar-simulated.svg)

**Main clock face** — big digits with the date tucked between the colon's dots:

![Simulated main screen](img/screen-simulated.svg)

*Both are simulations, not photographs — generated by
...
harder-edged and its diagonals dither; the type here is the viewer's, not the
embedded Saira and Montserrat. They are drawn from the source so they cannot
quietly fall out of date, which is what happened to the photo they replaced.*

## Features

- **A calendar screen**: Monday-first month grid with today boxed and adjacent-month days a size down, an analogue dial beside it, the four-day forecast underneath and a quote strip at the foot. Arrow buttons page through the months
