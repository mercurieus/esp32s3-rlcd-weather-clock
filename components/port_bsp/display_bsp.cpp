...
/* Always ships the whole packed panel buffer, fixed to the full-screen
   ...
   was implemented and hardware-tested twice on 2026-08-30 - once before
   Lvgl_Refresh()'s post-send settling delay was restored (an invalid
   test), once cleanly after. Neither showed a measurable benefit: the
   panel's visible settle cost appears roughly fixed per send, not
   proportional to the addressed window's area. See git history and
